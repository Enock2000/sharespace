...
export default function LoginPage() {
    ...
    const handle2FAVerify = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                // Fetch role again to redirect correctly (or pass it from previous step if we stored it)
                // For safety, let's just fetch or rely on the previous fetch. 
                // Actually, we need to know where to redirect. 
                // Let's just fetch the user role again or store it in state. Refetching is safer.
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Only stop loading if we didn't trigger 2FA (which handles its own loading state transition)
            // But wait, if we setRequires2FA(true), we set loading false there.
            // If we redirect, we probably want to keep loading true until redirect happens? 
            // The router.push is async but usually we don't await it.
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-400 mt-2">
                        {requires2FA ? "Two-Factor Authentication Required" : "Welcome back! Sign in to continue"}
                    </p>
                </div>
                ...
                <div className="glass rounded-2xl p-8 shadow-2xl">
                    <h2 className="text-2xl font-bold text-white mb-6">
                        {requires2FA ? "Enter Code" : "Sign In"}
                    </h2>
                    ...
                    {!requires2FA ? (
                        ...
                        <form onSubmit={handle2FAVerify} className="space-y-5 animate-in slide-in-from-right-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-2">
                                    Authenticator Code
                                </label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all tracking-[0.5em] text-center font-mono text-lg"
                                        placeholder="000000"
                                        required
                                        autoFocus
                                    />
