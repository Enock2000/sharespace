...
import { onValue, ref, push, set, update, serverTimestamp, query, orderByChild, limitToLast } from "firebase/database";
...
export function ChatWindow({ channelId, type }: ChatWindowProps) {
    ...
    const handleSendMessage = async (content: string, file?: File) => {
        ...
        if (type === "channel") {
            ...
            // For DMs, we also want to update the last_message field for the list view
            const updates: any = {};
            updates[`chat_messages/${channelId}/${messageId}`] = newMessage;
    ...
    // Update read status when channel is opened
    useEffect(() => {
        if (channelId) {
            localStorage.setItem(`last_read_${channelId}`, Date.now().toString());
        }
    }, [channelId, messages]); // Update whenever messages change so we mark as read up to "now" basically
